...
var getHandler = (exports.getHandler = function (options, proxy) {
  ...
  return function (req, res) {
    ...
    if (
      req.url.match(
        /^\/steamcommunity.com\/workshop\/browse\/\?appid=286160/
      ) == null
    ) {
      // This app is only for searching TTS mods on the Steam workshop, return 404 for all other requests
      res.writeHead(404, "Invalid destination", cors_headers);
      res.end(
        "Invalid destination: " +
          req.url +
          "\nT  his app is only for searching TTS mods on the Steam workshop"
      );
      return;
    }

    //TODO: Connect this to a Mongo database and cache Steam results for lower traffic
